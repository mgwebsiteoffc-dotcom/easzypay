...
class ShopifyService
{
    ...
    public function createOrder(CheckoutSession $session, array $paymentData): array
    {
        try {
            ...
            $total       = (int) ($session->subtotal ?? 0);
            $totalString = number_format($total / 100, 2, '.', '');
...
if (!empty($paidCur)) {
    $noteAttributes[] = ['name' => 'Customer Currency', 'value' => $paidCur];
}
...
if (!empty($noteAttributes)) {
    $orderPayload['note_attributes'] = $noteAttributes;
            ...
            $orderPayload = [
                ...
                // 'note_attributes' => array_filter([
                //     $piStr     ? ['name' => 'Stripe Payment Intent', 'value' => $piStr]     : null,
                //     $chargeStr ? ['name' => 'Stripe Charge',         'value' => $chargeStr] : null,
                //     ($brandStr || $last4Str) ? ['name' => 'Card', 'value' => "{$brandStr} ****{$last4Str}"] : null,
                //     ['name' => 'Wallet',            'value' => $walletStr],
                //     ['name' => 'Customer Currency', 'value' => $paidCur],
                //     ['name' => 'Checkout Platform', 'value' => 'EaszyPay'],
                // ]),
            ];
            ...
            if (!empty($address1)) {
                ...
                $orderPayload['shipping_address'] = $addr;
                $orderPayload['billing_address']  = $addr;
            }
    ...
    // Inject Theme Button
    // ================================================================
    public function injectThemeSnippet(string $content): array
    {
        try {
            $themes = $this->rest('GET', 'themes.json');
            $active = null;
            foreach ($themes['themes'] ?? [] as $t) {
                if ($t['role'] === 'main') { $active = $t; break; }
            }
            if (!$active) throw new \RuntimeException('No active theme');

            $this->rest('PUT', "themes/{$active['id']}/assets.json", [
                'asset' => ['key' => 'snippets/easzypay-button.liquid', 'value' => $content],
            ]);

            return ['success' => true, 'theme_id' => $active['id']];
        } catch (\Throwable $e) {
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }
